...
def fhog(image, cell_size=4, alfa=0.2):
    """Compute 31-dim FHOG features in H x W x 31 layout."""
    if image is None:
    ...
    mapp = {}
    mapp = getFeatureMaps(img, int(cell_size), mapp)
    mapp = normalizeAndTruncate(mapp, alfa)
    mapp = PCAFeatureMaps(mapp)

    h = int(mapp['sizeY'])
    w = int(mapp['sizeX'])
    c = int(mapp['numFeatures'])
    feat = mapp['map'].reshape((h, w, c)).astype(np.float32, copy=False)

    if c != 31:
        raise RuntimeError(f"Unexpected FHOG channels: {c}, expected 31")
    return feat
